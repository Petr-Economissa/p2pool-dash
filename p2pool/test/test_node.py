...
from p2pool.gvidon import data as gvidon_data, networks, worker_interface
from p2pool.util import deferral, jsonrpc, math, variable

class gvidond(object): # can be used as p2p factory, p2p protocol, or rpc jsonrpc proxy
    def __init__(self):
        ...
        self.headers = {0x132b9afeca5e9a2fdf4477338df6dcff1342300240bc70397c4bb: {
            ...
            'bits': gvidon_data.FloatingInteger(bits=0x1b0d642e, target=0x44b9f20000000000000000000000000000000000000000000000L),
        }}
...
class MiniNode(object):
    ...
    def start(cls, net, factory, gvidond, peer_ports, merged_urls):
        self = cls()
        
        self.n = node.Node(factory, gvidond, [], [], net)
        yield self.n.start()
