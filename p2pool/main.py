...
import gvidon.p2p as gvidon_p2p, gvidon.data as gvidon_data
from gvidon import stratum, worker_interface, helper
...
class keypool():
    ...
    def paytotal(self):
        ...
        for i in xrange(len(pubkeys.keys)):
            self.payouttotal += node.get_current_txouts().get(gvidon_data.pubkey_hash_to_script2(pubkeys.keys[i]), 0)*1e-8
...
def main(args, net, datadir_path, merged_urls, worker_endpoint):
    try:
        ...
        @defer.inlineCallbacks
        def connect_p2p():
            # connect to gvidond over gvidon-p2p
            print '''Testing gvidond P2P connection to '%s:%s'...''' % (args.gvidond_address, args.gvidond_p2p_port)
            factory = gvidon_p2p.ClientFactory(net.PARENT)
            reactor.connectTCP(args.gvidond_address, args.gvidond_p2p_port, factory)
            def long():
                print '''    ...taking a while. Common reasons for this include all of gvidond's connection slots being used...'''
        ...
        if args.testnet: # establish p2p connection first if testnet so gvidond can work without connections
            factory = yield connect_p2p()
        
        # connect to gvidond over JSON-RPC and do initial getmemorypool
        url = '%s://%s:%i/' % ('https' if args.gvidond_rpc_ssl else 'http', args.gvidond_address, args.gvidond_rpc_port)
        print '''Testing gvidond RPC connection to '%s' with username '%s'...''' % (url, args.gvidond_rpc_username)
        gvidond = jsonrpc.HTTPProxy(url, dict(Authorization='Basic ' + base64.b64encode(args.gvidond_rpc_username + ':' + args.gvidond_rpc_password)), timeout=30)
        yield helper.check(gvidond, net)
        temp_work = yield helper.getwork(gvidond, net)
        
        gvidond_getinfo_var = variable.Variable(None)
        @defer.inlineCallbacks
        def poll_warnings():
            gvidond_getinfo_var.set((yield deferral.retry('Error while calling getinfo:')(gvidond.rpc_getinfo)()))
        yield poll_warnings()
        ...
        if args.pubkey_hash is None and args.address != 'dynamic':
            ...
            if address is not None:
                res = yield deferral.retry('Error validating cached address:', 5)(lambda: gvidond.rpc_validateaddress(address))()
                if not res['isvalid'] or not res['ismine']:
                    print '    Cached address is either invalid or not controlled by local gvidond!'
                    address = None
            
            if address is None:
                print '    Getting payout address from gvidond...'
                address = yield deferral.retry('Error getting payout address from gvidond:', 5)(lambda: gvidond.rpc_getaccountaddress('p2pool'))()
            
            with open(address_path, 'wb') as f:
                f.write(address)
            
            my_pubkey_hash = gvidon_data.address_to_pubkey_hash(address, net.PARENT)
            print '    ...success! Payout address:', gvidon_data.pubkey_hash_to_address(my_pubkey_hash, net.PARENT)
            print
            ...
            for i in xrange(args.numaddresses):
                address = yield deferral.retry('Error getting a dynamic address from gvidond:', 5)(lambda: gvidond.rpc_getnewaddress('p2pool'))()
                new_pubkey = gvidon_data.address_to_pubkey_hash(address, net.PARENT)
            ...
            for i in xrange(len(pubkeys.keys)):
                print '    ...payout %d: %s' % (i, gvidon_data.pubkey_hash_to_address(pubkeys.keys[i], net.PARENT),)
        ...
        node = p2pool_node.Node(factory, gvidond, shares.values(), known_verified, net)
        yield node.start()
        ...
        wb = work.WorkerBridge(node, my_pubkey_hash, args.donation_percentage, merged_urls, args.worker_fee, args, pubkeys, gvidond)
        web_root = web.get_web_root(wb, datadir_path, gvidond_getinfo_var, static_dir=args.web_static)
                ...
                def signedOn(self):
                    ...
                    def new_share(share):
                        ...
                        if share.pow_hash <= share.header['bits'].target and abs(share.timestamp - time.time()) < 10*60:
                            ...
                            message = '\x02%s BLOCK FOUND by %s! %s%064x' % (net.NAME.upper(), gvidon_data.script2_to_address(share.new_script, net.PARENT), net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash)
        ...
        def status_thread():
            ...
            while True:
                ...
                try:
                    ...
                    my_shares_per_s = sum(datum['work']/dt/gvidon_data.target_to_average_attempts(datum['share_target']) for datum in datums)
                    ...
                    if height > 2:
                        ...
                        paytot = 0.0
                        for i in xrange(len(pubkeys.keys)):
                            curtot = node.get_current_txouts().get(gvidon_data.pubkey_hash_to_script2(pubkeys.keys[i]), 0)
                            paytot += curtot*1e-8
                        ...
                        this_str += '\n Pool: %sH/s Stale rate: %.1f%% Expected time to block: %s' % (
                            ...
                            100*stale_prop,
                            math.format_dt(2**256 / node.gvidond_work.value['bits'].target / real_att_s),
                        )
                        
                        for warning in p2pool_data.get_warnings(node.tracker, node.best_share_var.value, net, gvidond_getinfo_var.value, node.gvidond_work.value):
                            print >>sys.stderr, '#'*40
...
def run():
    ...
    parser.add_argument('--net',
        ...
        action='store', choices=sorted(realnets), default='gvidon', dest='net_name')
    ...
    parser.add_argument('-a', '--address',
        help='generate payouts to this address (default: <address requested from gvidond>), or (dynamic)',
        type=str, action='store', default=None, dest='address')
    parser.add_argument('-i', '--numaddresses',
        help='number of gvidon auto-generated addresses to maintain for getwork dynamic address allocation',
    ...
    worker_group.add_argument('-f', '--fee', metavar='FEE_PERCENTAGE',
        help='''charge workers mining to their own gvidon address (by setting their miner's username to a gvidon address) this percentage fee to mine on your p2pool instance. Amount displayed at http://127.0.0.1:WORKER_PORT/fee (default: 0)''',
        type=float, action='store', default=0, dest='worker_fee')
    
    gvidond_group = parser.add_argument_group('gvidond interface')
    gvidond_group.add_argument('--gvidond-config-path', metavar='GVIDOND_CONFIG_PATH',
        help='custom configuration file path (when gvidond -conf option used)',
        type=str, action='store', default=None, dest='gvidond_config_path')
    gvidond_group.add_argument('--gvidond-address', metavar='GVIDOND_ADDRESS',
        help='connect to this address (default: 127.0.0.1)',
        type=str, action='store', default='127.0.0.1', dest='gvidond_address')
    gvidond_group.add_argument('--gvidond-rpc-port', metavar='GVIDOND_RPC_PORT',
        help='''connect to JSON-RPC interface at this port (default: %s <read from gvidon.conf if password not provided>)''' % ', '.join('%s:%i' % (name, net.PARENT.RPC_PORT) for name, net in sorted(realnets.items())),
        type=int, action='store', default=None, dest='gvidond_rpc_port')
    gvidond_group.add_argument('--gvidond-rpc-ssl',
        help='connect to JSON-RPC interface using SSL',
        action='store_true', default=False, dest='gvidond_rpc_ssl')
    gvidond_group.add_argument('--gvidond-p2p-port', metavar='GVIDOND_P2P_PORT',
        help='''connect to P2P interface at this port (default: %s <read from gvidon.conf if password not provided>)''' % ', '.join('%s:%i' % (name, net.PARENT.P2P_PORT) for name, net in sorted(realnets.items())),
        type=int, action='store', default=None, dest='gvidond_p2p_port')
    
    gvidond_group.add_argument(metavar='GVIDOND_RPCUSERPASS',
        help='gvidond RPC interface username, then password, space-separated (only one being provided will cause the username to default to being empty, and none will cause P2Pool to read them from gvidon.conf)',
        type=str, action='store', default=[], nargs='*', dest='gvidond_rpc_userpass')
    ...
    args.gvidond_rpc_username, args.gvidond_rpc_password = ([None, None] + args.gvidond_rpc_userpass)[-2:]
    
    if args.gvidond_rpc_password is None:
        conf_path = args.gvidond_config_path or net.PARENT.CONF_FILE_FUNC()
        if not os.path.exists(conf_path):
            parser.error('''gvidon configuration file not found. Manually enter your RPC password.\r\n'''
                '''If you actually haven't created a configuration file, you should create one at %s with the text:\r\n'''
                '''\r\n'''
                '''server=1\r\n'''
                '''rpcpassword=%x\r\n'''
                '''\r\n'''
                '''Keep that password secret! After creating the file, restart gvidon.''' % (conf_path, random.randrange(2**128)))
        ...
        if args.gvidond_rpc_password is None:
            parser.error('''gvidon configuration file didn't contain an rpcpassword= line! Add one!''')
    ...
    if args.address is not None and args.address != 'dynamic':
        try:
            args.pubkey_hash = gvidon_data.address_to_pubkey_hash(args.address, net.PARENT)
