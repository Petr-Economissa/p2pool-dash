...
import p2pool
from p2pool.gvidon import data as gvidon_data, script, sha256
...
class Share(object):
    ...
    small_block_header_type = pack.ComposedType([
        ...
        ('bits', gvidon_data.FloatingIntegerType()),
    ...
    share_info_type = pack.ComposedType([
        ...
        ('max_bits', gvidon_data.FloatingIntegerType()),
        ('bits', gvidon_data.FloatingIntegerType()),
        ('timestamp', pack.IntType(32)),
    ...
    def generate_transaction(cls, tracker, share_data, block_target, desired_timestamp, desired_target, ref_merkle_link, desired_other_transaction_hashes_and_fees, net, known_txs=None, last_txout_nonce=0, base_subsidy=None):
        ...
        max_bits = gvidon_data.FloatingInteger.from_target_upper_bound(pre_target3)
        bits = gvidon_data.FloatingInteger.from_target_upper_bound(math.clip(desired_target, (pre_target3//30, pre_target3)))
        ...
        for tx_hash, fee in desired_other_transaction_hashes_and_fees:
            if tx_hash in tx_hash_to_this:
                ...
                if known_txs is not None:
                    this_size = gvidon_data.tx_type.packed_size(known_txs[tx_hash])
        ...
        weights, total_weight, donation_weight = tracker.get_cumulative_weights(previous_share.share_data['previous_share_hash'] if previous_share is not None else None,
            ...
            65535*net.SPREAD*gvidon_data.target_to_average_attempts(block_target),
        )
        ...
        if payments is not None:
            for obj in payments:
                pm_script = gvidon_data.address_to_script2(obj['payee'],net.PARENT)
        ...
        this_script = gvidon_data.pubkey_hash_to_script2(share_data['pubkey_hash'])
        ...
        share_info = dict(
            ...
            abswork=((previous_share.abswork if previous_share is not None else 0) + gvidon_data.target_to_average_attempts(bits.target)) % 2**128,
        )
        ...
        def get_share(header, last_txout_nonce=last_txout_nonce):
            ...
            share = cls(net, None, dict(
                ...
                hash_link=prefix_to_hash_link(gvidon_data.tx_type.pack(gentx)[:-32-8-4], cls.gentx_before_refhash),
    ...
    def get_ref_hash(cls, net, share_info, ref_merkle_link):
        return pack.IntType(256).pack(gvidon_data.check_merkle_link(gvidon_data.hash256(cls.ref_type.pack(dict(
    ...
    def __init__(self, net, peer_addr, contents):
        ...
        self.new_script = gvidon_data.pubkey_hash_to_script2(self.share_data['pubkey_hash'])
        ...
        merkle_root = gvidon_data.check_merkle_link(self.gentx_hash, self.merkle_link)
        self.header = dict(self.min_header, merkle_root=merkle_root)
        self.pow_hash = net.PARENT.POW_FUNC(gvidon_data.block_header_type.pack(self.header))
        self.hash = self.header_hash = net.PARENT.BLOCKHASH_FUNC(gvidon_data.block_header_type.pack(self.header))
    ...
    def check(self, tracker):
        ...
        if gvidon_data.hash256(gvidon_data.tx_type.pack(gentx)) != self.gentx_hash:
            raise ValueError('''gentx doesn't match hash_link''')
        
        if gvidon_data.calculate_merkle_link([None] + other_tx_hashes, 0) != self.merkle_link:
    ...
    def should_punish_reason(self, previous_block, bits, tracker, known_txs):
        ...
        if other_txs is None:
            ...
            all_txs_size = sum(gvidon_data.tx_type.packed_size(tx) for tx in other_txs)
            if all_txs_size > 1000000:
                return True, 'txs over block size limit'
            
            new_txs_size = sum(gvidon_data.tx_type.packed_size(known_txs[tx_hash]) for tx_hash in self.share_info['new_transaction_hashes'])
...
class WeightsSkipList(forest.TrackerSkipList):
    ...
    def get_delta(self, element):
        from p2pool.gvidon import data as gvidon_data
        share = self.tracker.items[element]
        att = gvidon_data.target_to_average_attempts(share.target)
...
class OkayTracker(forest.Tracker):
    def __init__(self, net):
        forest.Tracker.__init__(self, delta_type=forest.get_attributedelta_type(dict(forest.AttributeDelta.attrs,
            work=lambda share: gvidon_data.target_to_average_attempts(share.target),
            min_work=lambda share: gvidon_data.target_to_average_attempts(share.max_target),
        )))
        self.net = net
    ...
    def think(self, block_rel_height_func, previous_block, bits, known_txs):
        ...
        if p2pool.DEBUG:
            print 'Desire %i shares. Cutoff: %s old diff>%.2f' % (len(desired), math.format_dt(time.time() - timestamp_cutoff), gvidon_data.target_to_difficulty(target_cutoff))
            for peer_addr, hash, ts, targ in desired:
                print '   ', None if peer_addr is None else '%s:%i' % peer_addr, format_hash(hash), math.format_dt(time.time() - ts), gvidon_data.target_to_difficulty(targ), ts >= timestamp_cutoff, targ <= target_cutoff
...
def get_stale_counts(tracker, share_hash, lookbehind, rates=False):
    ...
    for share in tracker.get_chain(share_hash, lookbehind - 1):
        res['good'] = res.get('good', 0) + gvidon_data.target_to_average_attempts(share.target)
        s = share.share_data['stale_info']
        if s is not None:
            res[s] = res.get(s, 0) + gvidon_data.target_to_average_attempts(share.target)
    if rates:
...
def get_expected_payouts(tracker, best_share_hash, block_target, subsidy, net):
    weights, total_weight, donation_weight = tracker.get_cumulative_weights(best_share_hash, min(tracker.get_height(best_share_hash), net.REAL_CHAIN_LENGTH), 65535*net.SPREAD*gvidon_data.target_to_average_attempts(block_target))
...
def get_desired_version_counts(tracker, best_share_hash, dist):
    ...
    for share in tracker.get_chain(best_share_hash, dist):
        res[share.desired_version] = res.get(share.desired_version, 0) + gvidon_data.target_to_average_attempts(share.target)
    return res

def get_warnings(tracker, best_share, net, gvidond_getinfo, gvidond_work_value):
    res = []
    ...
    if gvidond_getinfo['errors'] != '':
        if 'This is a pre-release test build' not in gvidond_getinfo['errors']:
            res.append('(from gvidond) %s' % (gvidond_getinfo['errors'],))
    
    version_warning = getattr(net, 'VERSION_WARNING', lambda v: None)(gvidond_getinfo['version'])
    if version_warning is not None:
        res.append(version_warning)
    
    if time.time() > gvidond_work_value['last_update'] + 60:
        res.append('''LOST CONTACT WITH GVIDOND for %s! Check that it isn't frozen or dead!''' % (math.format_dt(time.time() - gvidond_work_value['last_update']),))
