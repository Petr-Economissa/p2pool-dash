...
class HeaderWrapper(object):
    ...
    def from_header(cls, header):
        return cls(gvidon_data.hash256(gvidon_data.block_header_type.pack(header)), header['previous_block'])
...
@defer.inlineCallbacks
def get_height_rel_highest_func(gvidond, factory, best_block_func, net):
    if '\ngetblock ' in (yield deferral.retry()(gvidond.rpc_help)()):
        @deferral.DeferredCacher
        @defer.inlineCallbacks
        def height_cacher(block_hash):
            try:
                x = yield gvidond.rpc_getblock('%x' % (block_hash,))
