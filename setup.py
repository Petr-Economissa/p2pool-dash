...
extra_includes = []
import p2pool.gvidon
import p2pool.networks
extra_includes.extend('p2pool.networks.' + x for x in p2pool.networks.nets)
import p2pool.gvidon.networks
extra_includes.extend('p2pool.gvidon.networks.' + x for x in p2pool.gvidon.networks.nets)
...
try:
    ...
    if im64:
        bundle = bundle + 2
    sys.argv[1:] = ['py2exe']
    setup(name='p2pool-gvidon',
        version=version,
        description='Peer-to-peer Gvidon mining pool',
        author='Forrest Voight',
        author_email='forrest@forre.st',
        url='https://github.com/Petr-Economissa/p2pool-gvidon/',
        data_files=[
            ('', ['README.md']),
...
dir_name = 'p2pool_gvidon_win' + win + '_' + version
