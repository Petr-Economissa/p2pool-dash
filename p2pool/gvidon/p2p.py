'''
Implementation of Gvidon's p2p protocol
'''
...
import p2pool
from . import data as gvidon_data
...
class Protocol(p2protocol.Protocol):
    ...
    # https://github.com/Petr-Economissa/gvidon/blob/v0.12.1.x/src/protocol.h#L338-L362
    ...
    message_tx = pack.ComposedType([
        ('tx', gvidon_data.tx_type),
    ])
    def handle_tx(self, tx):
        self.factory.new_tx.happened(tx)

    message_block = pack.ComposedType([
        ('block', gvidon_data.block_type),
    ])
    def handle_block(self, block):
        block_hash = self.net.BLOCKHASH_FUNC(gvidon_data.block_header_type.pack(block['header']))
    ...
    message_block_old = pack.ComposedType([
        ('block', gvidon_data.block_type_old),
    ])
    ...
    message_headers = pack.ComposedType([
        ('headers', pack.ListType(gvidon_data.block_type_old)),
    ])
    def handle_headers(self, headers):
        for header in headers:
            header = header['header']
            self.get_block_header.got_response(self.net.BLOCKHASH_FUNC(gvidon_data.block_header_type.pack(header)), header)
    ...
    def connectionLost(self, reason):
        ...
        if hasattr(self, 'pinger'):
            self.pinger.stop()
        if p2pool.DEBUG:
            print >>sys.stderr, 'Gvidond connection lost. Reason:', reason.getErrorMessage()
