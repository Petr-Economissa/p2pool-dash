...
from p2pool.gvidon import data as gvidon_data, helper, height_tracker
...
class P2PNode(p2p.Node):
    ...
    def handle_bestblock(self, header, peer):
        if self.node.net.PARENT.POW_FUNC(gvidon_data.block_header_type.pack(header)) > header['bits'].target:
        ...
        def _(share):
            ...
            def spread():
                if (self.node.get_height_rel_highest(share.header['previous_block']) > -5 or
                    self.node.gvidond_work.value['previous_block'] in [share.header['previous_block'], share.header_hash]):
...
class Node(object):
    def __init__(self, factory, gvidond, shares, known_verified_share_hashes, net):
    ...
    def start(self):
        ...
        self.gvidond_work = variable.Variable((yield helper.getwork(self.gvidond, self.net)))
        @defer.inlineCallbacks
        def work_poller():
            while stop_signal.times == 0:
                flag = self.factory.new_block.get_deferred()
                try:
                    self.gvidond_work.set((yield helper.getwork(self.gvidond, self.net, self.gvidond_work.value['use_getblocktemplate'])))
                except:
                    log.err()
        ...
        def handle_header(new_header):
            ...
            if not (self.net.PARENT.POW_FUNC(gvidon_data.block_header_type.pack(new_header)) <= self.gvidond_work.value['bits'].target):
                return
            gvidond_best_block = self.gvidond_work.value['previous_block']
            if (self.best_block_header.value is None
                or (
                    new_header['previous_block'] == gvidond_best_block and
                    self.net.PARENT.BLOCKHASH_FUNC(gvidon_data.block_header_type.pack(self.best_block_header.value)) == gvidond_best_block
                ) # new is child of current and previous is current
                or (
                    self.net.PARENT.BLOCKHASH_FUNC(gvidon_data.block_header_type.pack(new_header)) == gvidond_best_block and
                    self.best_block_header.value['previous_block'] != gvidond_best_block
        ...
        def poll_header():
            if self.factory.conn.value is None:
                return
            handle_header((yield self.factory.conn.value.get_block_header(self.gvidond_work.value['previous_block'])))
        self.gvidond_work.changed.watch(lambda _: poll_header())
        ...
        self.get_height_rel_highest = yield height_tracker.get_height_rel_highest_func(self.gvidond, self.factory, lambda: self.gvidond_work.value['previous_block'], self.net)
        
        self.best_share_var = variable.Variable(None)
        self.desired_var = variable.Variable(None)
        self.gvidond_work.changed.watch(lambda _: self.set_best_share())
        self.set_best_share()
        
        # setup p2p logic and join p2pool network
        
        # update mining_txs according to getwork results
        @self.gvidond_work.changed.run_and_watch
        def _(_=None):
            new_mining_txs = {}
            added_known_txs = {}
            for tx_hash, tx in zip(self.gvidond_work.value['transaction_hashes'], self.gvidond_work.value['transactions']):
        ...
        def _(share):
            ...
            if block is None:
                print >>sys.stderr, 'GOT INCOMPLETE BLOCK FROM PEER! %s gvidon: %s%064x' % (p2pool_data.format_hash(share.hash), self.net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash)
                return
            helper.submit_block(block, True, self.factory, self.gvidond, self.gvidond_work, self.net)
            print
            print 'GOT BLOCK FROM PEER! Passing to gvidond! %s gvidon: %s%064x' % (p2pool_data.format_hash(share.hash), self.net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash)
            print
    ...
    def set_best_share(self):
        best, desired, decorated_heads, bad_peer_addresses = self.tracker.think(self.get_height_rel_highest, self.gvidond_work.value['previous_block'], self.gvidond_work.value['bits'], self.known_txs_var.value)
	...
	#i = 210240
	real_subsidy = self.gvidond_work.value['subsidy']
	#while i <= self.gvidond_work.value['height']:
		#real_subsidy = real_subsidy*92.9/100
		#i = i + 210240
        # Use value from getblocktemplate's result.
        if self.gvidond_work.value['payment_amount'] >= 0 :
            real_pay = real_subsidy - self.gvidond_work.value['payment_amount']
            return p2pool_data.get_expected_payouts(self.tracker, self.best_share_var.value, self.gvidond_work.value['bits'].target, real_pay, self.net)
            
	if self.gvidond_work.value['height'] > 158000+((576*30)* 17):
		real_pay = (real_subsidy)*40/100
        elif self.gvidond_work.value['height'] > 158000+((576*30)* 15):
            real_pay = (real_subsidy)*42.5/100
        elif self.gvidond_work.value['height'] > 158000+((576*30)* 13):
            real_pay = (real_subsidy)*45/100
        elif self.gvidond_work.value['height'] > 158000+((576*30)* 11):
            real_pay = (real_subsidy)*47.5/100
        elif self.gvidond_work.value['height'] > 158000+((576*30)* 9):
            real_pay = (real_subsidy)*50/100
        elif self.gvidond_work.value['height'] > 158000+((576*30)* 7):
            real_pay = (real_subsidy)*52.5/100
        elif self.gvidond_work.value['height'] > 158000+((576*30)* 6):
            real_pay = (real_subsidy)*55/100
        elif self.gvidond_work.value['height'] > 158000+((576*30)* 5):
            real_pay = (real_subsidy)*57.5/100
        elif self.gvidond_work.value['height'] > 158000+((576*30)* 4):
            real_pay = (real_subsidy)*60/100
        elif self.gvidond_work.value['height'] > 158000+((576*30)* 3):
            real_pay = (real_subsidy)*62.5/100
        elif self.gvidond_work.value['height'] > 158000+((576*30)* 2):
            real_pay = (real_subsidy)*65/100
        elif self.gvidond_work.value['height'] > 158000+((576*30)* 1):
